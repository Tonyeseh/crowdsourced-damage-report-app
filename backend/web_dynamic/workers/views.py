...
def worker_home_page(current_user):
    ...
    for job in current_user.jobs:
        ...
        outstanding_jobs = [
            job for job in current_user.jobs if job.status == "In Progress"]
...
def get_all_jobs(current_user):
    ...
    damages = [job for job in working_on.values() if job.worker_id ==
               current_user.id]
