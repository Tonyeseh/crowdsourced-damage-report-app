...
def user_token_required(func):
    ...
    def decorated(*args, **kwargs):
        ...
        if not token:
            flash("Please login in to continue", category='danger')
        ...
        try:
            ...
            if current_user is None:
                flash("Could not login you in. Try again!", category='danger')
                return redirect('login')
...
def worker_token_required(func):
    ...
    def decorated(*args, **kwargs):
        """decorated function"""
        token = None
        if request.cookies.get('worker_access_token'):
            token = request.cookies.get('worker_access_token')
